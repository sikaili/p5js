function Particle(x,y,m){
  ...
  this.steering = function(p,set,radius,m){
  ...
  this.separation = function(particles){
    ...
    for (var j=0;j<particles.length;j++){
      this.disRange = (this.r+particles[j].r)/diss;
      ...
      if (count>0){
        targetAll.div(count);
        targetAll.normalize();
        targetAll.mult(this.maxSpeed/diss);
  ...
  this.display = function(colorS,colorF,sum){
      fill(colorF);
      stroke(colorS);
      ...
      rotate(theta);
      Eyes(sum+this.pos.x/20+constrain(mouseX/13,0,600),rand2-(this.mass)/300,rand1+(this.mass-1)/800-mouseY/1000,this.mass);
      pop();
  }
  function Eyes(number,z,d,size){
    for (var i = 0; i < number; i+=7){
      stroke(random(100,255),100)
