...
function setup() {
  ...
  loadSound("assets/dance.m4a", songLoad);
...
function draw() {
  ...
  if (peakDetect.isDetected) {
    ...
    if (state == 0 && state1 === 0) {
      ...
      background(220 + constrain(sum1, 0, 50), back*2);
    } else {
      background(0, sum1);
  ...
  if (state == -1 || state1 == -1) {
    ...
    text("Saturation Chinoise", 0.50 * windowWidth, 0.3 * windowHeight + 130);
