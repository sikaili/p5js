...
function setup() {
  ...
  loadSound("assets/ELECCSYN2.m4a",songLoad);
...
function draw() {
  var colorParticle = color(random(200,255),150);
  if (state==1||state==2){
    colorParticle = color(floor(random(1)*255),180);
  }
      ...
      if (peakDetect.isDetected) {
        ...
        nn = -1;
          if (state === 0) {
            ...
            stroke(50,50,200,50+back*3)
            background(255,back);
// finger
          }else {
            background(0,40);
            // stroke(random(50, 200), random(50, 200), random(50, 800));
            stroke(5,5,200-floor(random(1)*125),200);
            background(random(200,255),40);
          }
      }
  ...
  for (i = 0; i < particles.length; i++) {
    ...
    particles[i].fluide();
    particles[i].update();
    particles[i].display(sum1 / 5,colorParticle);
  }
  pop();
  // tranlate letters
  push();
  ...
  // 
    ...
    push();
    fill(255, map(sum, 200, 255, 0, 255));
    rotate(i * 2 * PI / sides);
    if(count1%2 == 1){
      text(name, 100, 0);
  ...
  // fill(150);
  // strokeWeight(0);
  // // text("http://skyl.fr skyl@me.com", 0.92 * windowWidth - 60, 0.9 * windowHeight, 100, 200);
  // pop();
  // start state 
  if (state == -1 || state1 == -1) {
    push();
    ...
    // text("Mixing : TANG Xiancheng (AB Studio)", 0.50 * windowWidth, 0.6 * windowHeight+190);
    // text("Sampler : LARGE Fréderic", 0.50 * windowWidth, 0.6 * windowHeight+160);
    text("Music, Code, Design : LI Sikai (skyl)", 0.50 * windowWidth, 0.6 * windowHeight+220);
    pop();
...
function Add(){
  for(var ee = 0; ee <20;ee++){
    var partica = new Particle(mouseX,mouseY,random(1,3),20);
    particles.push(partica);
  }
