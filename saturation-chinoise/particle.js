function Particle(x, y, m, r) {
  this.arr = '八 勹 匕 冫 卜 厂 刀 刂 儿 二 匚 阝 丷 几 卩 冂 力 冖 凵 人 亻 入 十 厶 亠 匸 讠 廴 又'.split('');
  ...
  this.display = function (m, sum, sum1) {
    ...
    if (Math.random() * sum > 120) {
      fill(-sum1 / 2 + sum + noise(this.pos.y, this.pos.y) * 150, sum1 / 3 + 150);
      if (Math.random() > 0.7) {
        fill(20,120);
      }

    } else {
      fill(noise(this.pos.x) * 255, 255 - noise(this.pos.x) * 255);
    }
    if (this.r > 100) {
      fill(200,20,20, 140 / this.mass);

      noStroke();
    }
    // line(this.pos.x,this.pos.y,this.pos.x,this.pos.y+this.r*3);
    noStroke();
    // textSize(this.r+sum1/10);
    textSize(this.r*2);
    
    text(this.arr[this.n],0,0)
    // ellipse(this.pos.x, this.pos.y, this.r + m, this.r + m)
    fill(0, 140);
    stroke(200,20,20, 140 / this.mass);
    strokeWeight(1);
